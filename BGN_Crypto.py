...
#Definim un primer p tal que ha de ser el resultat de 4*l*n on l és un nombre natural i n=q*r
def generar_p(q: int, r: int) -> int:
    """Devuelve un primo p = 4*l*(q*r) - 1 con p % 3 == 2."""
    n = q * r
    l = 1
    while True:
        p = 4 * l * n - 1
        if gmpy2.is_prime(p) and p % 3 == 2:
            return p
        l += 1
...
class BGN:
    ...
    def encrypt(self, m: int):
        while True:
            ...
            C = m * self.P + t * self.Q
            if not (C.x is None and C.y is None):  # evita ∞
                return C
    ...
    def sub(self, C1, C2):
        t0 = random.randint(1, self.n - 1)
        if (C2.x is None) and (C2.y is None):
            return C1 + t0 * self.Q  # re-aleatoriza C1
        return C1 - C2 + t0 * self.Q

    def scalar_mult(self, C, k: int):
        t0 = random.randint(1, self.n - 1)
        return k * C + t0 * self.Q


#Funció per trobar punts d'ordre n
def _punts_amb_ordre(curve, ordre_total: int):
...
#Definim el nostre sistema BGN amb totes les variables
def construir_bgn(q: int, r: int):
    n = q * r
    p = generar_p(q, r)
    ...
    S = curve_Fp2.point(field_Fp2.from_coefficients(0),field_Fp2.from_coefficients(1))

    
    # Punts d'ordre n
    punts_n = _punts_amb_ordre(curve, n)
    #Triam 2 punts aleatoris d'ordre n
    P1, P2 = random.sample(punts_n, 2)
    #P ha de ser d'ordre n
    P = Point(P1.x, P1.y, curve)
    #Q ha de ser d'ordre q (Q=[r]Q' on Q' és d'ordre n)
    Q_tilde = Point(P2.x, P2.y, curve)
    Q = r * Q_tilde  # Q de orden q

    pairing = partial(modified_weil_pairing, curve_Fp2, n=n, S=S)
    bgn = BGN(curve, P, Q, q, r, n, pairing)
    dades = {"p": p, "n": n, "curve": curve, "P": P, "Q": Q}
    return bgn, dades 
